#!/usr/bin/env python3
"""
Fetch January 2026 price data for comprehensive testing.

Downloads 15-minute price data for all days in January 2026 and saves
each day as a separate JSON file in test_data/january_2026/.
"""
...
DATA_DIR = Path('tests/test_data/january_2026')
...
async def fetch_all_january_prices():
    """Fetch prices for all days in January 2026."""
    fetcher = PriceFetcher()
    semaphore = asyncio.Semaphore(MAX_WORKERS)
    
    # Generate all dates in January 2026
    dates = [date(2026, 1, day) for day in range(1, 32)]
    
    logger.info(f"Starting to fetch prices for {len(dates)} days in January 2026")
...
async def main():
    """Main entry point."""
    try:
        await fetch_all_january_prices()
